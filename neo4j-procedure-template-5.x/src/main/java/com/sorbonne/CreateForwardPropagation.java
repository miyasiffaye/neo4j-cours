...
public class CreateForwardPropagation {
    ...
    public Stream<CreateForwardPropagation.CreateResult> createForwardPropagation() {
        try (Transaction tx = db.beginTx()) {
            ...
            // Requête Cypher
            String cypherQuery = "MATCH (row_for_inputs:Row {type: 'inputsRow'})-[inputsValue_R:CONTAINS]->(input:Neuron {type: 'input'})\n" +
            ...
            tx.commit(); // Assurez-vous que la transaction est validée

            return Stream.of(new CreateForwardPropagation.CreateResult("ok"));

        } catch (Exception e) {
            // Enregistrer l'erreur dans les logs Neo4j
            log.error("Error in createForwardPropagation procedure: " + e.getMessage(), e);
